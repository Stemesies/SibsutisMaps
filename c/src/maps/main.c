...
int main()
{
  ...
  printf("path: %d\nspeed: %d\n",
         graph->graph_matrix[hashtab_lookup(table, "Kolyvan")][hashtab_lookup(table, "Pikhtovka")].len,
         graph->graph_matrix[hashtab_lookup(table, "Pikhtovka")][hashtab_lookup(table, "Kolyvan")].speed);
